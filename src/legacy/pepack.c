...
static bool generic_packer(pe_ctx_t *ctx, uint64_t entrypoint)
{
    ...
    for (size_t i = 0; i < LIBPE_SIZEOF_ARRAY(invalid_flags); i++) {
        if (section->Characteristics & (uint32_t) invalid_flags[i]) {
            flags_count++;
        }
    }
...
static bool match_peid_signature(const unsigned char *data, char *sig)
{
    ...
    while (*sig) {
        ...
        byte = (unsigned char) strtoul((char *) byte_str, NULL, 16);
...
static bool compare_signature(const unsigned char *data, uint64_t ep_offset,
                              FILE *dbfile, char *packer_name,
                              size_t packer_name_len)
{
    if (! dbfile || ! data) {
        return false;
    }
    ...
    while (fgets(buff, MAX_SIG_SIZE, dbfile)) {
        ...
        if (*buff == '[' && *(buff + len - 2) == ']') {
            *(buff + len - 2) = '\0'; // remove square brackets
            ...
            packer_name[packer_name_len - 1]
                = '\0'; // Guarantee it's Null-terminated.
        }
...
int pepack(int argc, char *argv[])
{
    ...
    if (! opendb(&dbfile, options)) {
        fprintf(stderr,
                "WARNING: without valid database file, %s will search in "
                "generic mode only\n",
                PROGRAM);
    }
...
#ifdef STANDALONE
int main(int argc, char **argv) { return pepack(argc, argv); }
#endif
