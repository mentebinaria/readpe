...
static void peres_build_node_filename(pe_ctx_t *ctx, char *output,
                                      size_t output_size,
...
static void
peres_restore_resource_icon(peres_resource_restore_t *restore,
...
static void peres_restore_resource(peres_resource_restore_t *restore,
                                   const pe_resource_entry_info_t *entry_info,
                                   void *raw_data_ptr, size_t raw_data_size)
{
    assert(restore != NULL);
    ...
    default:
        goto fallback_untouched;
    case RT_ICON:
        peres_restore_resource_icon(restore, entry_info, raw_data_ptr,
                                    raw_data_size);
        break;
    }
...
static void peres_save_resource(pe_ctx_t *ctx, const pe_resource_node_t *node,
                                bool namedExtract)
{
    // UNUSED(ctx);
    assert(node != NULL);
...
void peres_save_all_resources(pe_ctx_t *ctx, const pe_resource_node_t *node,
                              bool namedExtract)
{
    if (node == NULL) {
        return;
...
void peres_show_version(pe_ctx_t *ctx, const pe_resource_node_t *node)
{
    ...
    LL_FOREACH(search_result.items, result_item)
    {
        ...
        if (version_node != NULL) {
            ...
            const void *data_ptr = LIBPE_PTR_ADD(
                ctx->map_addr,
                32 + data_offset); // TODO(jweyrich): The literal 32 refers to
                                   // the size of the
...
static void peres_generate_stats(peres_stats_t *stats,
                                 const pe_resource_node_t *node)
{
    if (node == NULL) {
        return;
