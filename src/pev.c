...
void pe_check_fake_entrypoint(PE_FILE *pe)
{
    // Wagner Barongello
    // 2012-03-29
    ...
    if (pe->optional_ptr->_32->AddressOfEntryPoint && pe->num_sections)
    {
        ...
        while (i < pe->num_sections && (ep < pe->sections_ptr[i]->VirtualAddress || ep >= pe->sections_ptr[i]->VirtualAddress + pe->sections_ptr[i]->Misc.VirtualSize))
            i++;

        if (i < pe->num_sections && !(pe->sections_ptr[i]->Characteristics & 0x00000020))
        {
            printf("Fake entrypoint detected!\n");
            printf("This application is either compressed or encrypted!\n");
            printf("Entrypoint is at section %d; %s\n", i, pe->sections_ptr[i]->Name);
        }
    }
