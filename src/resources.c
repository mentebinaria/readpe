...
/*
    ...
    resources.c - retrive informations and binary data of resources
...
typedef struct {
    ...
    union {
        ...
        uint16_t biXHotspot; // CUR - Horizontal coord of the hotspot in number
                             // of pixels from the left
    } u0;
    union {
        uint16_t biBitCount; // ICO - Number of bits per pixel
        uint16_t biYHotspot; // CUR - Vertical coord of the hotspot in number of
                             // pixels from the top
    } u1;
    uint32_t biSizeImage; // Size of image data in bytes
    uint32_t biOffBits;   // Offset of BMP or PNG data from the beggining of the
                          // ICO/CUR file
} ICODIRENTRY;
#pragma pack(pop)
...
static void print_resource_node(const pe_resource_node_t *node)
{
    ...
    case LIBPE_RDT_RESOURCE_DIRECTORY: {
        const IMAGE_RESOURCE_DIRECTORY *const resourceDirectory
    ...
    case LIBPE_RDT_DIRECTORY_ENTRY: {
        const IMAGE_RESOURCE_DIRECTORY_ENTRY *const directoryEntry
        ...
        snprintf(value, MAX_MSG, "%x",
                 directoryEntry->u1.data.OffsetToDirectory);
    ...
    case LIBPE_RDT_DATA_STRING: {
        const IMAGE_RESOURCE_DATA_STRING_U *const dataString
            = node->raw.dataString;
...
static void print_resource_nodes(const pe_resource_node_t *node)
{
    if (node == NULL) {
        return;
    }
...
static pe_resource_node_t *get_root_node(pe_ctx_t *ctx)
{
    ...
    return resources->root_node;
}

static void build_resource_node_filename(char *output, size_t output_size,
                                         const pe_resource_node_t *node)
{
    ...
    for (pe_resource_level_e level = LIBPE_RDT_LEVEL1; level <= node->dirLevel;
        ...
        if (dir_entry_node->raw.directoryEntry->u0.data.NameIsString) {
            snprintf(partial_path, sizeof(partial_path), "%s ",
                     dir_entry_node->name);
        } else {
            const pe_resource_entry_info_t *match
                = pe_resource_entry_info_lookup(
                    dir_entry_node->raw.directoryEntry->u0.data.NameOffset);
            if (match != NULL && level == LIBPE_RDT_LEVEL1) {
                snprintf(partial_path, sizeof(partial_path), "%s ",
                         match->name);
            } else {
                snprintf(
                    partial_path, sizeof(partial_path), "%04x ",
                    dir_entry_node->raw.directoryEntry->u0.data.NameOffset);
            }
        }
...
static void print_resource_node_list(const pe_resource_node_t *node)
{
...
static void print_resource_list(const pe_resource_node_t *node)
{
    ...
    print_resource_node_list(node);

    print_resource_list(node->childNode);
                                  ...
                                  void *raw_data_ptr, size_t raw_data_size)
{
    ...
    size_t   written = 0;
    uint8_t *buffer
        = malloc_s(sizeof(ICOFILEHEADER) + sizeof(ICODIRENTRY) + raw_data_size);

#define PERES_APPEND(dst, src, size)                                           \
...
static void restore_resource(struct stored_resource         *resource,
                             const pe_resource_entry_info_t *entry_info,
                             void *raw_data_ptr, size_t raw_data_size)
{
...
fallback_untouched:
    ...
    resource->buffer      = raw_data_ptr;
    resource->size        = raw_data_size;
}

static void save_resource(pe_ctx_t *ctx, const pe_resource_node_t *node,
                          bool namedExtract)
{
    assert(node != NULL);
    ...
    if (entry_info != NULL) {
        if (asprintf(&dirName, "%s/%s", g_resourceDir, entry_info->dir_name)
            < 0) {
            abort();
        }
        // snprintf(dirName, sizeof(dirName), "%s/%s", g_resourceDir,
        // entry_info->dir_name);
    } else {
        const size_t len = strlen(g_resourceDir);
        dirName          = malloc(len);
        memcpy(dirName, g_resourceDir, len);
    ...
    if (namedExtract) {
        ...
        build_resource_node_filename(fileName, sizeof(fileName), node);
        if (asprintf(&relativeFileName, "%s/%s%s", dirName, fileName,
                     entry_info != NULL ? entry_info->extension : ".bin")
            < 0) {
            abort();
        }
    } else {
        if (asprintf(&relativeFileName,
                     "%s/"
                     "%" PRIu32 "%s",
                     dirName, name_node->raw.directoryEntry->u0.data.NameOffset,
                     entry_info != NULL ? entry_info->extension : ".bin")
            < 0) {
            abort();
        }
    ...
    free(dirName);
    // printf("DEBUG: raw_data_offset=%#llx, raw_data_size=%ld,
    // relativeFileName=%s\n", raw_data_offset, raw_data_size,
    // relativeFileName);

    struct stored_resource resource = {0};
    restore_resource(&resource, entry_info, raw_data_ptr, raw_data_size);
    ...
    fwrite(resource.buffer, resource.size, 1, fp);
...
static void save_all_resources(pe_ctx_t *ctx, const pe_resource_node_t *node,
                               bool namedExtract)
{
    if (node == NULL) {
        return;
    ...
    if (node->type == LIBPE_RDT_DATA_ENTRY && node->dirLevel == 3) {
        save_resource(ctx, node, namedExtract);
    }
...
static void generate_resource_stats(struct resource_stats    *stats,
                                    const pe_resource_node_t *node)
{
    if (node == NULL) {
        return;
    ...
    if (node->childNode) {
        generate_resource_stats(stats, node->childNode);
    }
...
static void print_resource_stats(const pe_resource_node_t *node)
{
    struct resource_stats stats = {0};
    generate_resource_stats(&stats, node);
...
{
    pe_resource_node_t *root_node = get_root_node(ctx);
    print_resource_nodes(root_node);
}

void print_resources_stats(pe_ctx_t *ctx)
{
    pe_resource_node_t *root_node = get_root_node(ctx);
    print_resource_stats(root_node);
}

void print_file_version(pe_ctx_t *ctx)
{
    pe_resource_node_t *root_node = get_root_node(ctx);
    print_resource_version(ctx, root_node);
}

void extract_all_resources(pe_ctx_t *ctx, bool named)
{
    pe_resource_node_t *root_node = get_root_node(ctx);
    save_all_resources(ctx, root_node, named);
}
