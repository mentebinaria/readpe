...
int plugins_load(const char *path)
{
    ...
    // TODO: Rewrite plugin to give struct with functions
    // FIX: Ugly way to do it!
    //*(void **)(&entry->plugin_loaded_fn) = dylib_get_symbol(library,
    //"plugin_loaded");
    //*(void **)(&entry->plugin_initialize_fn) = dylib_get_symbol(library,
    //"plugin_initialize");
    //*(void **)(&entry->plugin_shutdown_fn) = dylib_get_symbol(library,
    //"plugin_shutdown");
    //*(void **)(&entry->plugin_unloaded_fn) = dylib_get_symbol(library,
    //"plugin_unloaded");
    entry->plugin_loaded_fn = dylib_get_symbol(library, "plugin_loaded");
    entry->plugin_initialize_fn
        = dylib_get_symbol(library, "plugin_initialize");
    ...
    if (entry->plugin_initialize_fn == NULL
        || entry->plugin_shutdown_fn == NULL) {
