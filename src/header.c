...
void print_optional_header(pe_ctx_t *ctx)
{
    ...
    case MAGIC_PE32_0:
    case MAGIC_PE32: {
        snprintf(s, MAX_MSG, "%#x (%s)", header->_32->Magic,
                 header->type == MAGIC_PE32_0 ? "PE32 ZERO" : "PE32");
        output("Magic number", s);
        ...
        snprintf(s, MAX_MSG, "%" PRIu16,
                 header->_32->MajorOperatingSystemVersion);
        output("Major version of required OS", s);

        snprintf(s, MAX_MSG, "%" PRIu16,
                 header->_32->MinorOperatingSystemVersion);
        ...
        snprintf(s, MAX_MSG, "Win32 version value:             %#x",
                 header->_32->Win32VersionValue);
        ...
        if (header->_32->Win32VersionValue == 0) {
            ...
            snprintf(s, MAX_MSG, "%u",
                     (header->_32->Win32VersionValue >> 8) & 0xff);
        }
        ...
        if (header->_32->Win32VersionValue == 0) {
            ...
            snprintf(s, MAX_MSG, "%u",
                     (header->_32->Win32VersionValue >> 16) & 0x3fff);
        }
        ...
        if (header->_32->Win32VersionValue == 0) {
            ...
            uint8_t platform_id
                = (uint8_t) (header->_32->Win32VersionValue >> 30);
            static const char *const win32_version_value_platform_id[4]
                = {"NT", "CE", "Win32s", "Win9x"};
            snprintf(s, MAX_MSG, "%u (%s)", platform_id,
                     win32_version_value_platform_id[platform_id]);
        }
        ...
        for (uint16_t i = 0, flag = 0x0001; i < 16; i++, flag <<= 1) {
            if (header->_32->DllCharacteristics & flag) {
                const char *characteristic_name
                    = pe_image_dllcharacteristic_name(flag);
        ...
        for (uint32_t i = 0, flag = 0x00000001; i < 32; i++, flag <<= 1) {
            if (header->_32->LoaderFlags & flag) {
                ...
                if (flag_name == NULL) {
                    snprintf(formatted_flag_name,
                             sizeof(formatted_flag_name) - 1, "UNKNOWN[%#x]",
                             flag);
    ...
    case MAGIC_PE64: {
        ...
        snprintf(s, MAX_MSG, "%" PRIu16,
                 header->_64->MajorOperatingSystemVersion);
        output("Major version of required OS", s);

        snprintf(s, MAX_MSG, "%" PRIu16,
                 header->_64->MinorOperatingSystemVersion);
        output("Minor version of required OS", s);
        ...
        if (header->_64->Win32VersionValue == 0) {
            ...
            snprintf(s, MAX_MSG, "%u",
                     (header->_64->Win32VersionValue >> 8) & 0xff);
        }
        output("Overwrite OS minor version", s);
        ...
        if (header->_64->Win32VersionValue == 0) {
            ...
            snprintf(s, MAX_MSG, "%u",
                     (header->_64->Win32VersionValue >> 16) & 0x3fff);
        }
        output("Overwrite OS build number", s);
        ...
        if (header->_64->Win32VersionValue == 0) {
            ...
            uint8_t platform_id
                = (uint8_t) (header->_64->Win32VersionValue >> 30);
            static const char *const win32_version_value_platform_id[4]
                = {"NT", "CE", "Win32s", "Win9x"};
            snprintf(s, MAX_MSG, "%u (%s)", platform_id,
                     win32_version_value_platform_id[platform_id]);
        ...
        for (uint16_t i = 0, flag = 0x0001; i < 16; i++, flag <<= 1) {
            if (header->_64->DllCharacteristics & flag) {
                ...
                if (pe_coff(ctx)->Characteristics & IMAGE_FILE_DLL) {
                    characteristic_name
                        = pe_dll_image_dllcharacteristic_name(flag);
                }
                if (characteristic_name == NULL) {
...
{
    const IMAGE_COFF_HEADER *header = pe_coff(ctx);
    if (! header) {
...
#else
            const char *characteristic_name
                = pe_image_characteristic_name(flag);
            char formatted_characteristic_name[32];
            if (characteristic_name == NULL) {
                snprintf(formatted_characteristic_name,
                         sizeof(formatted_characteristic_name) - 1,
