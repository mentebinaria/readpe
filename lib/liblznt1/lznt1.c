	...
	while ( offset != limit ) {
		...
		if ( tag & 1 ) {
			...
			if ( offset + sizeof ( *tuple ) > limit ) {
				//printf ( "LZNT1 compressed value overrun at ""%#zx\n", offset );
				return -1;
			}
			tuple = ( data + offset );
			offset += sizeof ( *tuple );
...
ssize_t lznt1_decompress ( const void *data, size_t len, void *buf ) {
	...
	while ( offset != len ) {
		...
		if ( ( offset + sizeof ( *header ) ) > len ) {
			//printf ( "LZNT1 block header overrun at %#zx\n", offset );
			return -1;
		}
		header = ( data + offset );
		...
		if ( LZNT1_BLOCK_COMPRESSED ( *header ) ) {
			...
			/* Compressed block */
			//printf ( "LZNT1 compressed block %#zx+%#zx\n", offset, block_len );
			...
			if ( ( offset + block_len ) > len ) {
				//printf ( "LZNT1 uncompressed block overrun at ""%#zx+%#zx\n", offset, block_len );
				return -1;
			}
			//printf ( "LZNT1 uncompressed block %#zx+%#zx\n", offset, block_len );
			if ( buf ) {
