...
void print_optional_header(PE_FILE *pe)
{
	...
	else if (pe->optional_ptr->_64)
	{
		...
		snprintf(s, MAX_MSG, "%#llx", pe->optional_ptr->_64->ImageBase);
		output("ImageBase", s);
		...
		snprintf(s, MAX_MSG, "%#llx", pe->optional_ptr->_64->SizeOfStackReserve);
		output("Size of stack to reserve", s);

		snprintf(s, MAX_MSG, "%#llx", pe->optional_ptr->_64->SizeOfStackCommit);
		output("Size of stack to commit", s);

		snprintf(s, MAX_MSG, "%#llx", pe->optional_ptr->_64->SizeOfHeapReserve);
		output("Size of heap space to reserve", s);

		snprintf(s, MAX_MSG, "%#llx", pe->optional_ptr->_64->SizeOfHeapCommit);
		output("Size of heap space to commit", s);
	}
}
